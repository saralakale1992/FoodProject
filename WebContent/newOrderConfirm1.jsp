...
<%
	...
	List<Orderstatus> o=bl.searcgstatus(status);
	System.out.println(o);
	
	//System.out.print("list"+l.get(0).getOrderstatus().getTotal());
	session.setAttribute("list1",o);
					...
					<table border="5" class="table table-bordered table-striped">
						...
						<c:forEach items="${sessionScope.list1}" var="l1">
							<tr>
							 ...
							 <td>${l1.registration.emailid}</td>
						
						  <td>${l1.total}</td>  
						   <td>${l1.item.status}</td>
						   <td><a href="ActivateServlet?id=${l1.item.id}">Confirm Order</a></td>
						      <td><a href="DeActiveServlet?id=${l1.item.id}">Pending Order</a></td>
